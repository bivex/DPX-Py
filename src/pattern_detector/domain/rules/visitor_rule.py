...
class VisitorPatternRule(BasePatternRule):
    ...
    def detect(self, model: CodeModel) -> list[Detection]:
        ...
        detections.extend(self._detect_visitor_interfaces(model))
        detections.extend(self._detect_element_accept_methods(model))
        return detections

    def _detect_visitor_interfaces(self, model: CodeModel) -> list[Detection]:
        results: list[Detection] = []
        for proto in model.all_protocols():
            det = self._analyze_visitor_proto(proto, model)
            if det:
                results.append(det)
        return results

    def _analyze_visitor_proto(self, proto: Any, model: CodeModel) -> Detection | None:
        visit_methods = [m for m in proto.methods if "visit" in m.name.lower()]
        if not (visit_methods and ("visitor" in proto.name.lower() or len(visit_methods) >= 2)):
            return None
    ...
    def _detect_element_accept_methods(self, model: CodeModel) -> list[Detection]:
        results: list[Detection] = []
        for rec in model.all_records():
            results.extend(self._analyze_element_record(rec))
        return results

    def _analyze_element_record(self, rec: Any) -> list[Detection]:
        results: list[Detection] = []
